            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {NAV_ITEMS.map((item) => (
                <a
                  key={item.label}
                  href={item.href}
                  className="font-sans text-sm font-medium text-slate-300 hover:text-nova-400 transition-colors duration-200"
                >
                  {item.label}
                </a>
              ))}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {NAV_ITEMS.map((item) => (
              <a
                ...
                className="block px-3 py-2 rounded-md text-base font-medium text-slate-300 hover:text-white hover:bg-zinc-800"
              >
                {item.label}
              </a>
            ))}
