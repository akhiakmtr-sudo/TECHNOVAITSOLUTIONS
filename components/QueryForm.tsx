...
export const QueryForm: React.FC = () => {
  ...
  return (
    <section id="query" className="py-24 bg-slate-950 relative overflow-hidden">
        {/* Background Gradients */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-nova-500/5 rounded-full blur-[100px]"></div>
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 md:p-10 shadow-2xl">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input 
                    ...
                    id="name"
                    required
                    className="w-full bg-slate-950 border border-white/10 rounded-lg px-4 py-3 text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-nova-500 transition-all"
              ...
              {loadingState === LoadingState.IDLE && (
                 <button 
                  type="submit"
                  className="w-full bg-gradient-to-r from-nova-600 to-purple-600 hover:from-nova-500 hover:to-purple-500 text-white font-bold py-4 rounded-lg transition-all transform hover:scale-[1.01] shadow-lg flex items-center justify-center gap-2"
                >
              ...
              {loadingState === LoadingState.LOADING && (
                <button disabled className="w-full bg-slate-800 text-slate-400 font-bold py-4 rounded-lg flex items-center justify-center gap-2 cursor-not-allowed">
            ...
            {loadingState === LoadingState.SUCCESS && aiResponse && (
              <div className="mt-8 animate-fade-in-up">
                <div className="bg-slate-950/80 border border-nova-500/30 rounded-xl p-6 relative overflow-hidden">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-white/10 pb-4">
                      ...
                      <span className={`font-bold px-3 py-1 rounded-full text-sm inline-block w-fit mt-1 md:mt-0 ${
                        aiResponse.estimatedComplexity === 'High' ? 'bg-red-500/20 text-red-400' :
                        aiResponse.estimatedComplexity === 'Medium' ? 'bg-yellow-500/20 text-yellow-400' :
                  ...
                  <div className="mt-6">
                    <button 
                      ...
                      className="w-full bg-white text-slate-900 hover:bg-slate-200 font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
                    >
            ...
            {loadingState === LoadingState.ERROR && (
               <div className="mt-6 p-4 bg-red-500/10 border border-red-500/30 rounded-lg flex items-center gap-3 text-red-200">
